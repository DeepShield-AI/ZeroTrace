...
import (
	...
	"github.com/zerotraceio/zerotrace/server/ingester/ingesterctl/cmd"
)
...
func Execute(version string) {
	root := &cobra.Command{
		Use:              "zerotrace-ctl",
		Short:            "zerotrace server tool",
		TraverseChildren: true,
	}

	var outputVersion bool
	root.PersistentFlags().BoolVarP(&outputVersion, "version", "v", false, "zerotrace-ctl version")
	root.PersistentFlags().StringP("ip", "i", common.GetDefaultRouteIP(), "zerotrace-server service ip")
	root.PersistentFlags().Uint32P("api-port", "", 30417, "zerotrace-server service node port")
	root.PersistentFlags().Uint32P("rpc-port", "", 30035, "zerotrace-server service grpc port")
	root.PersistentFlags().Uint32P("svc-port", "", 20417, "zerotrace-server service http port")
	root.PersistentFlags().Uint32P("org-id", "", common.DEFAULT_ORG_ID, "organization id")
	root.PersistentFlags().DurationP("timeout", "", time.Second*30, "zerotrace-ctl timeout")
