...
* 注意：当 zerotrace-agent 携带的 org_id 在 ClickHouse 中没有对应的数据库时，
...
* 因此，被删除的 org_id 不要立即复用，以避免被删除的组织中有一些 zerotrace-agent 仍然在运行且未进入逃逸状态时，会将数据写入到复用此 org_id 的数据库中。
...
* Note: When the org_id carried by the Zerotrace-agent does not have a corresponding database in ClickHouse,
...
* Therefore, the deleted org_id should not be reused immediately to avoid some Zerotrace-agents in the deleted organization still running and not entering the escape state,
