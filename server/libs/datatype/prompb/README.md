...
In order to allow other logic in zerotrace-server (such as querier) to continue to use the original pb.go file, we modified the package in the code optimized here to avoid conflicts with the original structure.
...
# change package from `prometheus` to `prometheus_zerotrace`
sed -i 's/\<prometheus\>/prometheus_zerotrace/g' ./*.proto ./*.pb.go
```
